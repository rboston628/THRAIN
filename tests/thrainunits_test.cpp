...
Calculation::OutputData setupFakeCalcData(units::Units unitType){
  ...
  CHECK_EQ(fakeData.unitset.G, 1.0);
  CHECK_EQ(fakeData.unitset.C, 2.0);
  CHECK_EQ(fakeData.unitset.base_length, 8.0);
  CHECK_EQ(fakeData.unitset.base_time, 7.0);
  CHECK_EQ(fakeData.unitset.base_mass, 6.0);
  units::format_units(fakeData);
  // assert nothing unchanging has changed
  CHECK_EQ(fakeData.mass, fakeMass);
  CHECK_EQ(fakeData.radius, fakeRadius);
  CHECK_EQ(fakeData.params, fakeParams);
...
void do_test_setUnits_noparams(units::Units unitType){
  ...
  CHECK_NOTHROW(units::format_units(fakeData1));
  // assert all safely zeroed out
  CHECK_EQ(fakeData1.mass, 0.0);
  CHECK_EQ(fakeData1.radius, 0.0);
  CHECK_EQ(fakeData1.zsurf, 0.0);
  CHECK_EQ(fakeData1.logg, 0.0);
  ...
  CHECK_NOTHROW(units::format_units(fakeData2));
  // assert all safely zeroed out
  CHECK_EQ(fakeData2.mass, 0.0);
  CHECK_EQ(fakeData2.radius, 0.0);
  CHECK_EQ(fakeData2.zsurf, 0.0);
  CHECK_EQ(fakeData2.logg, 0.0);
}    
...
void do_test_setUnits(units::Units unitType){
  ...
  CHECK_EQ(fakeData.unitset.G, theseUnits.G);
  CHECK_EQ(fakeData.unitset.C, theseUnits.C);
  CHECK_EQ(fakeData.unitset.base_length, theseUnits.base_length);
  CHECK_EQ(fakeData.unitset.base_time, theseUnits.base_time);
  CHECK_EQ(fakeData.unitset.base_mass, theseUnits.base_mass);
  ...
  CHECK_DELTA(fakeData.zsurf, zsurf, 1.e-4);
...
TEST_CASE_FIXTURE(SetUpTearDown, "units_astro") {
    ...
    CHECK( fakeData.unitset.C == C_astro );
    CHECK( fakeData.unitset.base_length == 1.0e5 );
    CHECK( fakeData.unitset.base_time == 1.0 );
    CHECK( fakeData.unitset.base_mass == MSOLAR );
}
...
TEST_CASE_FIXTURE(SetUpTearDown, "units_geo") {
    ...
    CHECK( fakeData.unitset.C == 1.0 );
    CHECK( fakeData.unitset.base_length == 1.0/C_CGS );
    CHECK( fakeData.unitset.base_time == 1.0 );
    CHECK( fakeData.unitset.base_mass == C_CGS/G_CGS );
}
...
TEST_CASE_FIXTURE(SetUpTearDown, "param_setters") {
    ...
    CHECK_DELTA( radius6, fakeData.radius, 1.e-10 );

    // test making mass from logg and R
    fakeData.logg = units::getLoggFromRM(fakeData);
    double mass7 = units::getMassFromRLogg(fakeData);
    CHECK_DELTA( mass7, fakeData.mass, 1.e-10 );  
}
}