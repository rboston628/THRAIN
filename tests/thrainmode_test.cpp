...
TEST_SUITE("Mode Base Tests [unit]") {
  ...
  TEST_CASE("thrainmode: compare_JCD_bad_lk") {
    ...
    // fail L too high
    CHECK_ISNAN(mode::compare_JCD(1.5, 4, 1, 0.01));
    // fail K too low
    CHECK_ISNAN(mode::compare_JCD(1.5, 2, 0, 0.01));
    // fail K too high
  ...
  TEST_CASE("thrainmode: compare_JCD"){
    ...
    for(std::size_t K=1; K<36; K++){
      ...
      CHECK_EQ(0.0, mode::compare_JCD(3.0, L, K, mode::JCD3_0[L-1][K-1]/nug));
      CHECK_EQ(0.0, mode::compare_JCD(4.0, L, K, mode::JCD4_0[L-1][K-1]/nug));
    }
    }
  } 
...
void do_test_mode_finder_fake_classes(
  ...
  CHECK_EQ(data.mode_num, Kexp.size());
  CHECK_EQ(data.k.size(), Kexp.size());
  CHECK_EQ(data.l.size(), Lexp.size());
  CHECK_EQ(data.k.size(), Lexp.size());
  for(std::size_t i=0; i<Kexp.size(); i++){
    CHECK_EQ(data.l[i], Lexp[i]);
    CHECK_EQ(data.k[i], Kexp[i]);
    CHECK_EQ(data.w[i], sqrt(double(w2exp[i])));
  }
}
} // namespace
...
TEST_SUITE("Controlled Mode Finder Tests [unit]") {
  ...
  void do_test_mode_finder_fake_classes(
    ...
    CHECK_EQ(data.mode_num, Kexp.size());
    CHECK_EQ(data.k.size(), Kexp.size());
    CHECK_EQ(data.l.size(), Kexp.size());
    CHECK_EQ(data.k.size(), Lexp.size());
    for(std::size_t i=0; i<Kexp.size(); i++){
      CHECK_EQ(data.l[i], Lexp[i]);
      CHECK_EQ(data.k[i], Kexp[i]);
