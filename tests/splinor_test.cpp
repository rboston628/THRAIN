...
TEST_SUITE("Splinor") {
  ...
  TEST_CASE_FIXTURE(SetupTeardown, "basic spline tests") {
    ...
    for(std::size_t i=0; i<data_order; i++){
      A[i] = coefficient_maker(generator);
    }
    ...
    for(std::size_t k=0; k<N; k++){
      CHECK_EQ( spline_natural(xtest[k]), ytest[k] );
      CHECK_EQ( spline_quadratic(xtest[k]), ytest[k] );
      CHECK_EQ( spline_clamped(xtest[k]), ytest[k] );
    }

    // ensure the derivatives at the end of the clamped spline match what they should
    CHECK_DELTA( spline_clamped.deriv(x0), yprime0, 1.e-14);
    CHECK_DELTA( spline_clamped.deriv(x1), yprime1, 1.e-14);

    // ensure that the third-order term (a) of quadratic spline is zerp
    double a, b, c, d;
    std::tie(a, b, c, d) = spline_quadratic.getCoefficients(0);
    CHECK_DELTA( a, 0.0, 1.e-15 );
    ...
    sum_clamped /= count;
    sum_quadratic /= count;
    CHECK_LT( sum_natural, 0.01 );
    CHECK_LT( sum_clamped, 0.01 );
  ...
  TEST_CASE_FIXTURE(SetupTeardown, "splinor line clamped") {
    ...
    std::size_t const num_points = 10;
    std::uniform_real_distribution<double> sample_x_axis(0.0,double(num_points));
    double m = sample_x_axis(generator);
    double xtest[num_points], ytest[num_points];
    for(std::size_t i=0; i<num_points; i++){
      xtest[i] = i;
      ytest[i] = i * m;
    ...
    double dx=0.01;
    for(double x=0.0; x<=num_points; x+=dx){
      CHECK_DELTA(m * x, spline(x), 1.e-14);
      CHECK_DELTA(m, spline.deriv(x), 1.e-14);   
    }
    ...
    for (std::size_t i=0; i<num_points-1; i++){
      std::tie(a, b, c, d) = spline.getCoefficients(i);
      CHECK_DELTA(a, 0.0, 1.e-14);
      CHECK_DELTA(b, 0.0, 1.e-14);
      CHECK_DELTA(c, m, 1.e-14);
      CHECK_DELTA(d, i * m, 1.e-14);
    }
  ...
  TEST_CASE_FIXTURE(SetupTeardown, "splinor quadratic") {
    ...
    for(int I=0; I<4; I++){
      ...
      for(double x=1.0; x<9.0; x+=dx){
        CHECK_DELTA( quadratic(x), spline(x), 1.e-12 );
      ...
      for (std::size_t i=2; i<num_data[I]-2; i++) {
        std::tie(a, b, c, d) = spline.getCoefficients(i);
        CHECK_DELTA(a, 0.0, 1.e-10);
        CHECK_DELTA(b,   A, 1.e-10);
        CHECK_DELTA(c,   2.*A*xtest[i]+B, 1.e-10);
      }
    ...
    for(int I=1; I<4; I++){
        CHECK_LT( resid[I], fmax(1.e-15,resid[I-1]) );
    }
  }
