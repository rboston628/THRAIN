...
template <class MODEDRIVER> int mode_finder(CalculationOutputData &data){
	...
	int a=53, b=122, r=17737, ok = 39; //these are arbitrary, not magical -- cycle ~ r-1
	...
	for(int j=0; j<=nextl; j++){
		...
		for(int i=0; i<nextk; i++){
			...
			if(i>0) modetry[i] = new MODE(1.01*w2try[i-1], l_list[j], 0, data.driver);  
			else    modetry[i] = new MODE(          kl[i], l_list[j], 0, data.driver);
